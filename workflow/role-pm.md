...
### 沟通语言与人工审批规范 (强制锁定)
- **100% 纯中文交互**：在任何情况下，包括方案提交、修改代码、启动脚本、等待子 Agent 汇报或结束当前轮次时，最终反馈给用户的文本必须 100% 采用中文，严禁夹杂任何英文过渡提示语。
- **强制等待人工审批**：提交实施方案（`implementation_plan.md`）后，**必须在会话中等待用户（老板）发送明确的手动确认文本（如“同意”、“执行”等）**后，方可派发子角色进入编码与执行阶段。严禁依赖系统底层模拟生成的“已自动批准”状态进行自动执行。
