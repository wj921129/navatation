...
3. **任务清单 (task.md) 强制包含 Push**：执行计划时，必须在创建 `task.md` 时将 `[ ] 自动推送代码到 dev 分支` 作为最后一项任务，防范遗忘。
4. **构建与自我修复**：
   - 代码编写完毕后，汇报前必须主动尝试运行本地编译/Lint 检查。
   - **[绝对红线]**：**如果报错，你必须自行诊断修复，绝不允许将明显报错（编译不通过/TS报错）的代码直接丢给老板。**
5. **代码推送拦截点 (CRITICAL)**：在完成本地验证无误后、向老板汇报之前，**必须**自动运行 `.\push-dev.bat` 推送代码。绝不能在未推送的情况下提前汇报 `walkthrough`。
6. **痕迹清理**：生成的任何测试脚本、临时日志、无用的冗余文件，必须在汇报前物理删除，并清理所有无关的后台任务。
...
- **日常暂存 (`dev` 分支)**：在完成一个闭环的修改且**本地验证通过后**，你必须**自动使用后台任务运行** [push-dev.bat](file:///e:/workspace/navatation/scripts/git/push-dev.bat) 脚本，将修改推送到 `dev`。为防止遗忘，每次创建 `task.md` 时必须将此操作列为清单的最终步。
