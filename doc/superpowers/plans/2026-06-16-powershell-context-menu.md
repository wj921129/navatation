...
**Goal:** 通过注册表脚本在 Windows 11 中恢复经典右键菜单并增加“在 PowerShell 中打开 (管理员)”功能，采用 Windows 终端 (wt.exe) 载入 Windows PowerShell，并实现自动提权与起始工作路径定位。

**Architecture:** 编写添加与移除注册表分支的 `.reg` 配置文件，并提供对应的自动化导入与清理测试验证脚本。

**Tech Stack:** Windows Registry, PowerShell, Batch, Windows Terminal
...
if ($shellCommand -notlike "*wt.exe*") {
