...
- **注释约束**: 接口及非私有方法必写中文 Javadoc，禁数字编号(如 1., ①)。所有类的头部必须添加包含创建日期和功能描述的类注释。
- **换行控制**：单行 ≤180 字符；左大括号不换行；连续空行 ≤1 行
- **类依赖**：禁用通配符导入，删除未使用的 `import`。禁止在业务方法中直接使用全限定包名+类名引用类（如 `java.util.List`），所有引用的类必须在文件头部通过 import 显式引入。
