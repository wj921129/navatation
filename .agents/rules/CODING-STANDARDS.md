...
> **本文件是 [AGENTS.md](../../AGENTS.md) 的子规范，聚焦于编码红线、安全约束与测试要求。**
...
- **[绝对红线] 严禁写任何自定义 CSS 或行内样式 (inline styles)**。必须且只能使用 Tailwind 提供的 utility classes，使用 `cn()` 进行样式合并。需要新组件时，**必须**先在 shadcn/ui 体系中查找。**只有当** shadcn/ui 不提供所需组件时，才允许自行封装，且自定义组件**必须遵循** shadcn/ui 的 API 风格和文件规范。
...
- **性能与安全红线 (CRITICAL)**：
  ...
  - **非空判定**：任何非空判定必须使用通用工具类（如 `StringUtils.isNotBlank`, `CollectionUtils.isNotEmpty` 等）。禁止手写 null/empty 判断逻辑。
- **日志与依赖规范**：
  ...
  - 所有的数据流、连接对象等必须使用 `try-with-resources` 语法进行自动化安全关闭。
...
- **错误处理架构**：
  - **前端**：所有网络请求必须统一通过 `api-client.ts` 的响应拦截器处理。业务错误（后端返回 code != 200）使用 `toast.error(msg)` 展示，非业务异常（网络超时等）使用通用兜底提示。禁止将底层系统异常直接暴露给用户。
