...
The decision map is a single compact Markdown file, one per planning effort, git-tracked alongside the project. It is the canonical artifact 鈥?the **whole map is loaded as context into every session**, so it must stay compact.
...
2. Write a new decision map 鈥?mostly fog, frontier identified, trivially-decidable entries resolved inline.
