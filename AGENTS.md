...
> **⚠️ 核心探索约束**：在定位代码、理解架构或查找上下文时，**必须优先调用 `codebase-memory` 系列 MCP 工具**（如 `search_graph`、`trace_call_path`、`get_architecture`、`search_code` 等），严禁在初期大量使用 `grep`、`Search` 或盲目跨文件读取以节省 Token 开销。
...
GitHub Issues. See `doc/agents/issue-tracker.md`.

### Triage labels

Default label vocabulary. See `doc/agents/triage-labels.md`.

### Domain docs

Single-context layout. See `doc/agents/domain.md`.
